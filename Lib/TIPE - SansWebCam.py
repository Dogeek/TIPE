...
def Snapshot(nom="image",device=0): #PREND UNE PHOTO
...
def FindContours(thresh,img): #####CETTE FONCTION TROUVE LES CHIFFRES
    contours,hierarchy = cv2.findContours(thresh,cv2.RETR_LIST,cv2.CHAIN_APPROX_SIMPLE)
    nb=0
    for cnt in contours:
        cnt=cv2.approxPolyDP(cnt,0.1*cv2.arcLength(cnt,True),True) #Approximation du contour
        if len(cnt)==4 and cv2.contourArea > 1000 and cv2.isContourConvex(cnt): ######PAS SUR KSA MARCHE########Retravailler cette condition pour ne selectionner que les chiffres les bons contours
...
def CodePostal(solutions):#MET LE RESULTAT DE LA RECONNAISSANCE SOUS FORME D'UN NOMBRE UNIQUE
    ...
    code_postal=0
    for i in range(l):
        code_postal+=nb[i]*(10**i)
    return code_postal



print(CodePostal(NumberRecognition()))
